...
class APIs {
    /**
     * Save the player balance using miguel economy mod methods.
     * @param {string} mcPlayer - The Minecraft Entity. Example Event.player.getMCEntity()
     * @param {float} balance - The new Balance to save.
     * @returns {void}
     */
    savePlayerBalanceMethod(x, y, z, particleType) {
        var economyCap = mcPlayer.getCapability(EconomyProvider.ECON_CAP).orElse(null);
        if (economyCap != null) {
            economyCap.setBalance(balance); 
        }
        ScoreboardHandler.updatePlayerScore(mcPlayer, balance);
        ScoreboardHandler.syncPlayerBalance(mcPlayer);
        return true;
    }
    /**
     * Return the balance of the player using miguel economy mod method.
     * @param {string} mcPlayer - The Minecraft Entity. Example Event.player.getMCEntity()
     * @returns {float} return the current player balance
     */
    getPlayerSavedBalanceMethod(mcPlayer) {
        var economyCap = mcPlayer.getCapability(EconomyProvider.ECON_CAP).orElse(null);
        if (economyCap != null) {
            return economyCap.getBalance();
        }
        return null;
    }
    /**
     ...
     * @param {string} source Name of the file 
     * @param {string} dir The Directory where the file at.
